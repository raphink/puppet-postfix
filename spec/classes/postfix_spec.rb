    ...
    context 'when setting parameters' do
      ...
      context "on #{distro}" do
        ...
        context 'when mailman is true' do
          ...
          it 'should include postfix::mailman' do
            should include_class('postfix::mailman')
          end
        end
        context 'when specifying a custom maincf_source' do
          let (:params) { {
            :maincf_source => 'the_parameter_value'
          } }
          it { should contain_file('/etc/postfix/main.cf').with(:source => 'the_parameter_value') }
        end
        context 'when specifying a custom mastercf_source' do
          let (:params) { {
            :mastercf_source => 'the_parameter_value'
          } }
          it { should contain_file('/etc/postfix/master.cf').with(:source => 'the_parameter_value') }
        end
      ...
      -o smtpd_client_restrictions=check_client_access,hash:/etc/postfix/access,reject",
        ...
        context 'when specifying mydesitination' do
         let (:params) { {
            :mydestination => 'the_parameter_value'
          } }
          it 'should do stuff' do
            pending 'this currently does nothing by itself. I believe this to be a flaw'
            subject.resources.each do |resource|
              p 
              p resource
            end
          end
        ...
        context 'when specifying mynetworks' do
         let (:params) { { :mynetworks => 'the_parameter_value' } }  
        ...
        context 'when specifying relayhost' do
          let (:params) { { :relayhost => 'the_specified_mailhost'} }
          it 'should do stuff' do
            pending 'this currently does nothing by itself. I believe this to be a flaw'
            subject.resources.each do |resource|
              p 
        ...
        context 'when specifying satellite' do
          ...
          it { should include_class('postfix::mta') }
          it { should include_class('postfix::satellite') }
        ...
        context 'when specifying smtp_listen' do
          let (:params) { { :smtp_listen => 'fluffybunny' } }
          it 'should update master.cf and create a listener at the desired location' do
            should contain_file('/etc/postfix/master.cf').with_content(/fluffybunny:smtp/)
          end
        end
